...
# ~11 meters — accept meaningful movement despite GPS accuracy / last_seen filters.
_COORD_CHANGE_THRESHOLD = 0.0001
# Minimum seconds between automatic burst requests on zone transitions.
_BURST_COOLDOWN_SEC = 60
...
class Life360DeviceTracker(
    ...
    def __init__(self, coordinator: MemberDataUpdateCoordinator, mid: MemberID) -> None:
        ...
        self._accepted_latitude: float | None = None
        self._accepted_longitude: float | None = None
        self._last_tracker_state: str | None = None
        self._last_burst_monotonic: float | None = None
        if self._data.loc:
            self._accepted_latitude = self._data.loc.details.latitude
            self._accepted_longitude = self._data.loc.details.longitude
    ...
    def force_update(self) -> bool:
        """Return True when accepted coordinates changed so map cards redraw."""
        if not self._data.loc or self._accepted_latitude is None:
            return False
        lat = self._data.loc.details.latitude
        lon = self._data.loc.details.longitude
        return (
            abs(lat - self._accepted_latitude) > _COORD_CHANGE_THRESHOLD
            or abs(lon - self._accepted_longitude) > _COORD_CHANGE_THRESHOLD
        )
    ...
    def _handle_coordinator_update(self, config_changed: bool = False) -> None:
        ...
        self._update_basic_attrs()
        prev_state = self.state if self._data.loc else None
        self._process_update()
        self._maybe_request_location_burst(prev_state)
    ...
    def _process_update(self) -> None:
        ...
        new_lat = self._data.loc.details.latitude
        new_lon = self._data.loc.details.longitude
        prev_lat = self._prev_data.loc.details.latitude
        prev_lon = self._prev_data.loc.details.longitude
        coords_changed = (
            abs(new_lat - prev_lat) > _COORD_CHANGE_THRESHOLD
            or abs(new_lon - prev_lon) > _COORD_CHANGE_THRESHOLD
        )
        place_changed = (
            self._data.loc.details.place != self._prev_data.loc.details.place
        )
        if coords_changed or place_changed:
            if bad_last_seen or bad_accuracy:
                _LOGGER.debug(
                    "%s: Accepting location update despite filter "
                    "(coords_changed=%s, place_changed=%s, bad_last_seen=%s, "
                    "bad_accuracy=%s)",
                    self,
                    coords_changed,
                    place_changed,
    ...
    def _maybe_request_location_burst(self, prev_state: str | None) -> None:
        """Request a Life360 burst when zone state changes (home/away/etc.)."""
        if not self._data.loc or prev_state is None:
            return

        new_state = self.state
        if new_state is None or new_state == prev_state:
            return

        now = time.monotonic()
        if (
            self._last_burst_monotonic is not None
            and now - self._last_burst_monotonic < _BURST_COOLDOWN_SEC
        ):
            return

        self._last_burst_monotonic = now
        _LOGGER.debug(
            "%s: Zone transition %s -> %s; requesting location burst",
            self,
            prev_state,
            new_state,
        )
        self.hass.async_create_task(self.update_location())
